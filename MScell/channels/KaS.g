...
//Activation constants for alphas and betas Shen et al., 2004 fig 3B (minf^2) and 3D (mtau), rates in ms
...
echo "make kA, qfactor=" {qfactorkAs}	
    for (c = 0; c < {xdivsFiner} + 1; c = c + 1)
        ...
        float minf = {1 / {1+{exp { {{x} - {vmh}} / {vmc} } }}}
        float hinf = {{1 / {1+{exp { {{x} - {vhh} - {hshift}} / {vhc} } }}}} //* {{a}*{hinf}+{1-{a}}}}
 		
//        float mtau = {{{taum0}  + { {Cmt} * {exp {  { { {x}-{vthm} }/{vtcm}}^2 * {-1.0} }}}} * {1e-3}}
        float mtau_sq = { {{x}-{vthm}}/{vtcm} }
        float mtau_exp = { exp { -{{mtau_sq} * {mtau_sq}} } }
        float mtau = {{{taum0} + {{Cmt} * {mtau_exp}}} * {1e-3}}

        float left = {{alpha} * {exp { -{{x}-{vth1}-{htaushift}}/{vtc1} }}}
        float right = {{beta} * {exp { {{x}-{vth2}-{htaushift}}/{vtc2} }}}
